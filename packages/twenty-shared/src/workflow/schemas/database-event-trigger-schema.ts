...
export const workflowDatabaseEventTriggerSchema = baseTriggerSchema
  .extend({
    ...
    settings: z.object({
      ...
      outputSchema: z
        .looseObject({})
        .describe(
          'Schema defining the output data structure. For database events, this includes the record that triggered the workflow, accessible via {{trigger.properties.after.fieldName}} (created/updated/upserted) or {{trigger.properties.before.fieldName}} (deleted).',
        ),
  ...
  .describe(
    'Database event trigger that fires when a record is created, updated, deleted, or upserted. The triggered record is accessible in workflow steps via {{trigger.properties.after.fieldName}} (created/updated/upserted) or {{trigger.properties.before.fieldName}} (deleted) — NOT {{trigger.object.fieldName}}.',
  );
