...
export class ObjectSystemSideEffectsOnDeleteSideEffectHandlerService extends MetadataSideEffectHandler(
  ...
  // link lives in the NAVIGATION payload), so this scans the command menu item
  // maps instead of walking an aggregator, until the target becomes a real
  // column (twentyhq/core-team-issues#2789). The payload is the authority, so
  // rows not yet re-owned onto getNavigationCommandUniversalIdentifier are
  // cascaded too and a row merely holding the derived identifier for another
  // object is spared.
