    ...
    if (!semver.satisfies(version, requiredVersionRange)) {
      return scope === 'workspace'
        ? {
            ...
            message: `App requires ERPilot server ${requiredVersionRange} but this workspace has only completed the upgrade to ${version}.`,
          }
        : {
            compatible: false,
            reason: 'INSTANCE_INCOMPATIBLE',
            message: `App requires ERPilot server ${requiredVersionRange} but this server is ${version}.`,
          };
    }
