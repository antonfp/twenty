...
import { createPostDocumentTool } from 'src/engine/api/mcp/tools/post-document.tool';
...
// Covers the MCP fast-follow: post_document must check canUpdateObjectRecords
// for the calling role before touching PostingService, via the same
// ErpObjectPermissionGuardService callback the resolver uses.
describe('createPostDocumentTool — permission check', () => {
  it('refuses when the calling role lacks the permission', async () => {
    ...
    const tool = createPostDocumentTool(
      postingService,
      WORKSPACE_ID,
