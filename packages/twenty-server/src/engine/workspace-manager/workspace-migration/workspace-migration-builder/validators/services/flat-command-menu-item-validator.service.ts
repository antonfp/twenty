...
export class FlatCommandMenuItemValidatorService {
  ...
  // A NAVIGATION item whose payload targets an object is the engine-owned
  // singleton navigation command of that object: it must be
  // isSystemSideEffect and must be the only one for the object (two of them
  // are two identical rows sharing a hotkey). Discrimination is path-first,
  // like the payload union: path-based NAVIGATION items share the engine key
  // but are a legitimate authoring surface and are not caught, an incidental
  // objectMetadataItemId next to their path included.
